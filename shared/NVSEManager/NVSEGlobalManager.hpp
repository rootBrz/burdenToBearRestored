...
public:
  ...
  bool RegisterPluginEventListener(
      const char *apSender,
      NVSEMessagingInterface::EventCallback apCallback) const;
  bool DispatchPluginEvent(uint32_t auiMessageType, void *apData,
                           uint32_t auiDataLength,
                           const char *apReceiver = nullptr) const;
};
